...
public sealed class SteamKitProductInfoSource : IProductInfoSource
{
    ...
    public static Task<SteamKitProductInfoSource> ConnectAnonymousAsync(TimeSpan timeout, CancellationToken ct) =>
        // Run the blocking connect+pump loop on a background thread so a caller on a UI
        // thread (e.g. WPF) doesn't freeze for up to `timeout` waiting for synchronous pumps.
        Task.Run(async () =>
        {
        ...
        IReadOnlyList<uint> appIds, CancellationToken ct)
    {
        ...
        if (!resultSet.Complete || resultSet.Failed)
            throw new IOException("PICS product info job incomplete/failed — will retry");
